...
Program Definition h_core0
  ...
  : prog :=
    ...
    (* ikp and okp *)
    syntax.iter 0 16 (fun i => ikp@'i <-
     (key[[i]] xor (val (int32 909522486%Z))));;;

    syntax.iter 0 16 (fun j => okp@'j <-
     (key[[j]] xor (val (int32 1549556828%Z))));;;

    (* Set up the first hash's input *)
    syntax.iter 0 16 (fun i => fstin@'i <- ikp[[i]]);;;
    syntax.iter 16 32 (fun j => fstin@'j <- msg[[''(j-16)]]);;;
    ...
    syntax.iter 0 16 (fun i => sndin@'i <- okp[[i]]);;;
    syntax.iter 16 24 (fun j => sndin@'j <- fstot[[''(j-16)]]);;;
    ...
    (* And finally... *)
    syntax.iter 0 8 (fun i => hout@'i <- sndot[[i]]);;;
done.
