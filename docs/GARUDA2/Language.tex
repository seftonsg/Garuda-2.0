...
%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \section{Compilation to Verilog}\label{sec:compile}
    ...
    \subsection{Compiling \sysname to the Intermediate}\label{sec:compile:compile}
      ...
      Assume all $\mathbf{C}$ are short for $\mathbf{C}_{(EXE,EXE',MEM,MEM')}$ if unspecified.
        ...
        \begin{figure}
          \begin{align*}
            ...
            \mathbf{C}[f = n]\
              =\
              &if\ EXE=n\ then\\
              &\quad write(EXE', read(EXE))\\
              &\quad else\ \mathbf{C}_{(EXE, EXE', -, -})[0]\\
              &if\ MEM=n\ then\\
              &\quad write(MEM', read(MEM))\\
              &\quad else\ \mathbf{C}_{(-, -, MEM, MEM'})[0]\\
            %Predicate Addition
            \mathbf{C}[\mathsf{test}(a + b)]\ 
              =\
              & (*Intermediate\ Execution\ Ports*)\\
              & let\ E_{ai}, E'_{ai}, E_{bi}, E'_{bi} = \text{new buf}\ in\\
              & (*Intermediate\ Memory\ Ports*)\\
              & let\ M_{ai}, M'_{ai}, M_{bi}, M'_{bi} = \text{new buf}\ in\\
              &\quad\ \ DeMux(EXE, E_{ai}, E_{bi})\ ||\\
              &\quad\ \ DeMux(MEM, M_{ai}, M_{bi})\\
              &let\     i_a = \mathbf{C}_{(E_{ai},E'_{ai},M_{ai},M'_{ai})}[a]\\
              &\quad\ \ i_b = \mathbf{C}_{(E_{bi},E'_{bi},M_{bi},M'_{bi})}[b]\\
              &in\ Mux(i_a\ \ i_b, (EXE',MEM'))\\
            %Predicate Product
